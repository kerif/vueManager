<template>
  <el-dialog :visible.sync="show" :title="$t('订单详情')" class="finish-details-container"
  @close="clear" width="85%">
        <div class="receiverMSg msg-top">
        </div>
        <h3 class="change-sty">{{$t('收货人信息')}}</h3>
        <el-form label-width="100px" :model="form" ref="form" class="demo-ruleForm"
...
export default {
  ...
  methods: {
    // 添加签收照片
    getList () {
      this.$request.photosData(this.id, {

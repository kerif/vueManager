<template>
  <el-dialog :visible.sync="show" :title="state === 'sms' ? $t('短信API服务预警设置') : $t('物流查询服务预警设置')" class="alert-settings-container" @close="clear"
  width="30%">
  <el-form :model="ruleForm" ref="ruleForm" label-width="80px">
    <el-form-item :label="$t('预警设置')">
      ...
      <span class="warning-sty" v-if="ruleForm.radio === 1">{{$t('低于预警次数将发送短信提醒')}}</span>
      <span class="warning-sty" v-else>{{$t('不再发送短信提示')}}</span>
    </el-form-item>
    <el-form-item :label="$t('预警次数')">
      <el-input-number class="add-counts" v-model="num" @change="handleChange" :min="1" :max="100"></el-input-number>
    </el-form-item>

<template>
  ...
  >
    ...
    <el-table :data="tableData" border style="width: 100%">
      ...
      <!-- 物品名称 -->
      <el-table-column prop="boxes_count" :label="$t('箱数')"> </el-table-column>
      <!-- 物品价值 -->
      <el-table-column
        prop="weight"
      ...
      <el-table-column prop="package_value" :label="$t('尺寸/体积')">
        <template slot-scope="scope">
          <span
            >{{ scope.row.length }}*{{ scope.row.width }}*{{ scope.row.height
            }}{{ `${localization.length_unit ? localization.length_unit : ''}` }}/{{
              scope.row.volume
            }}</span
          >
        </template>
      </el-table-column>
      <el-table-column prop="created_at" :label="$t('完成时间')"> </el-table-column>
      <!-- 佣金 -->
      <el-table-column
        prop="amount"
    ...
    <div>
      <p>
        {{ $t('合计') }}：{{ tableData.length }}{{ $t('个订单') }} {{ $t('总佣金')

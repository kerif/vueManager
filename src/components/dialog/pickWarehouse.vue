<template>
  <el-dialog :visible.sync="show" :title="state === 'add' ? $t('新增货位'): $t('编辑货位')"
  class="dialog-pick-warehouse" @close="clear" width="60%">
  <div class="tips">
...
export default {
  ...
  methods: {
    // 获得数据
    getDetails () {
      this.$request.getAlongLocation(this.areaId, {
        XStationId: this.id
      }).then(res => {
        if (res.ret) {
    ...
    // 解锁 或 锁定
    lockLocation (id, val) {
      this.$request.locationLocked(id, val, {
        XStationId: this.id
      }).then(res => {
        if (res.ret) {
          this.$notify({
    ...
    confirm () {
      ...
      if (this.state === 'add') {
        ...
        this.$request.locationEdit(this.areaId, {
          XStationId: this.id,
          ...this.location
          // reusable: Number(this.location.reusable)
        }).then(res => {

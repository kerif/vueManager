  ...
  >
    ...
    <el-table class="table-sty" :data="tableData" border style="width: 100%">
      ...
      <el-table-column :label="$t('付款状态')">
        <template slot-scope="scope">
          <span v-if="scope.row.on_delivery_status > 0 && scope.row.on_delivery_status !== 2">{{
            '货到付款（未付款）'
          }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('操作')" width="190px">
        <template slot-scope="scope">
          ...
          <el-button class="btn-green" @click="goPhont(scope.row.id)">{{
            $t('添加签收照片')
          }}</el-button>
        </template>
      </el-table-column>
    ...
    >
      ...
      <div>
        <h3>{{ $t('原始包裹清单') }}</h3>
        <el-table :data="pakeageData" class="data-list" border stripe v-loading="tableLoading">
...
export default {
  ...
  methods: {
    ...
    confirmShip() {
      ...
      this.strArr = this.tableData
        .filter(item => item.on_delivery_status > 0 && item.on_delivery_status !== 2)
        .map(item => item.order_sn)
      if (this.strArr.length) {
        this.$confirm(
          this.$t(`该订单${this.strArr.toString()}为货到付款订单（未付款），是否确认收款并签收`),
          this.$t('提示'),
          {

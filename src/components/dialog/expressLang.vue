<template>
  <el-dialog :visible.sync="show" :title="line.name + '的翻译内容'" class="dialog-express-lang" @close="clear">
    <div class="lang-sty">
      <p>
...
export default {
  data () {
    return {
      ...
      rules: {
        name: [
          { required: true, message: this.$t('请输入快递公司名称'), trigger: 'blur' }
        ]
      },
      line: {
        id: '',
  ...
  methods: {
    // 获取订单增值的语言
    getLang () {
      this.$request.expressLang(this.line.id, {

    ...
    <div class="pay-message receiverMSg" v-if="this.$route.params.state === 'refund'">
      ...
      <el-row :gutter="20">
        <el-col :span="9">
          ...
          <span v-if="form.refund_type === 2">{{$t('其它方式')}}</span><br/>
...
export default {
  ...
  methods: {
    ...
    reviewPass () {
      if (this.$route.params.state === 'pay') {
        dialog({ type: 'financeReview', name: this.$route.params.state, id: this.form.order_id, state: 'pass', tranAmount: this.form.tran_amount }, () => {
          this.$router.go(-1)
        })
      } else {
    ...
    reviewReject () {
      if (this.$route.params.state === 'pay') {
        dialog({ type: 'financeReview', name: this.$route.params.state, id: this.form.order_id, state: 'reject' }, () => {
          this.$router.go(-1)
        })
      } else {
        dialog({ type: 'financeReview', name: this.$route.params.state, id: this.$route.params.id, state: 'reject' }, () => {
          this.$router.go(-1)
        })
      }
    }

<template>
  <div>
    <el-table :data="tableData" border style="width: 100%; margin-bottom: 10px">
      ...
      <el-table-column prop="payment_type_name" :label="$t('支付类型')"> </el-table-column>
      <el-table-column prop="serial_no" :label="$t('流水号')" width="200"> </el-table-column>
      <el-table-column prop="transfer_account" :label="$t('转账账号')"> </el-table-column>
      ...
      <el-table-column prop="order_number" :label="$t('相关订单')"> </el-table-column>
      ...
      <el-table-column :label="$t('操作')">
        <template slot-scope="scope">
          <el-button size="mini" class="btn-purple btn-sty" @click="goDetails(scope.row.id)">{{
            $t('详情')
          }}</el-button>
        </template>
      </el-table-column>

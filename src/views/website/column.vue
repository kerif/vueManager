      ...
      <el-table-column type="expand">
        <template slot-scope="props">
          <el-table :data="props.row.orders">
            ...
            <!-- 排序 -->
            <el-table-column :label="$t('排序')" prop="sort_index">
            </el-table-column>
            ...
            <el-table-column :label="$t('操作')" width="300">
              <template slot-scope="scope">
                <!-- 编辑 -->
                <el-button class="btn-dark-green btn-margin" @click="editSecondColumn(scope.row.id)">{{$t('编辑')}}</el-button>
                <!-- 删除 -->
                <el-button @click="deleteData(scope.row.id)" class="btn-light-red">{{$t('删除')}}</el-button>
              </template>
            </el-table-column>
          </el-table>
        </template>
        ...
        <el-table-column :label="$t('操作')" width="300">
          <template slot-scope="scope">
            <!-- 编辑 -->
            <el-button class="btn-dark-green btn-margin" @click="editColumn(scope.row.id, scope.row.has_children)">{{$t('编辑')}}</el-button>
            <!-- 风险提示 -->
            <el-button class="btn-main" @click="addSecond(scope.row.id)">{{$t('新增子栏目')}}</el-button>
            <!-- 删除 -->
            <el-button class="btn-light-red btn-margin" @click="deleteData(scope.row.id)">{{$t('删除')}}</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    editColumn (id, children) {
      if (children === 1) { // 当存在子栏目时
        this.$router.push({ name: 'editFirstWeb', params: { id: id } })
      } else {
        this.$router.push({ name: 'editColumn', params: { id: id, children: children, state: 'first' } })
      }
    },
    // 新增子栏目
    addSecond (id) {

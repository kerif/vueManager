<template>
  <div class="withdrawal-list-container">
    ...
    >
      ...
      <el-table-column
        :label="$t('提现金额') + this.localization.currency_unit"
        prop="amount"
      ></el-table-column>
      ...
      <el-table-column :label="$t('操作')">
        <template slot-scope="scope">
          <el-button
            ...
            @click="inviteWithdrawal(scope.row.user.id, scope.row.id)"
            >{{ $t('审核') }}</el-button
          >
          <el-button
            v-else
            class="btn-deep-purple optionBtn"
            @click="withdrawalDetail(scope.row.user.id, scope.row.id)"
            >{{ $t('详情') }}</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    getList() {
      ...
      this.$request
        .agentsWithdraws(this.$route.params.id, {

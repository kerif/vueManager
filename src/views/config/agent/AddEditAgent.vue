      ...
      <el-form ref="params" :model="user" label-width="140px" label-position="left" :rules="rules">
        <el-col :lg="12" class="baseInfo">
          ...
          <el-row :gutter="20">
            ...
            <el-button class="choose" @click="chooseUser" v-if="!this.$route.query.id">{{
              $t('选择')
            }}</el-button>
          </el-row>
          <!-- 密码 -->
...
export default {
  data() {
    return {
      ...
      rules: {
        // 必填项校验
        user_id: [{ required: true, message: this.$t('请输入代理用户'), trigger: 'change' }],
        agent_name: [{ required: true, message: this.$t('请输入代理名称'), trigger: 'change' }],
        contact_name: [{ required: true, message: this.$t('请输入联系人'), trigger: 'change' }],
        contact_email: [

<template>
  <div class="arriavl">
    ...
    <div>
      <el-table :data="tableData" border stripe ref="table" height="calc(100vh - 270px)" class="data-list country" v-loading="tableLoading" @selection-change="selectionChange">
        ...
        <el-table-column :label="$t('启用状态')" prop="status" width="150">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.status" @change="changeStatus(scope.row.id,scope.row.status)" :active-text="$t('开')" :inactive-text="$t('关')" :active-value="1" :inactive-value="0" active-color="#13ce66" inactive-color="gray">
            </el-switch>
          </template>
        </el-table-column>
        ...
        <el-table-column :label="$t('到件先知状态')" width="150">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.auto_status" @change="changeTransfer(scope.row.id,scope.row.status)" :active-text="$t('开')" :inactive-text="$t('关')" :active-value="1" :inactive-value="0" active-color="#13ce66" inactive-color="gray">
            </el-switch>
          </template>
        </el-table-column>
        <el-table-column :label="item.name" v-for="item in formatLangData" :key="item.id" align="center">
          <template slot-scope="scope">       
            <span v-if="scope.row['trans_' + item.language_code]" class="el-icon-check icon-sty" @click="forecastLang(scope.row, item)"></span>
            <span v-else class="el-icon-plus icon-sty" @click="forecastLang(scope.row, item)"></span>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    ForecastExpressList() {
      this.$request.ForecastExpressList().then(res => {
        if (res.ret) {

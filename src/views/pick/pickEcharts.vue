...
export default {
  ...
  methods: {
    ...
    getPie () {
      ...
      this.$request.getPickEcharts(params).then(res => {
        if (res.ret) {
          ...
          let xData = res.data.daily.map(item => item.days)
          let yData = res.data.daily.map(item => item.counts)

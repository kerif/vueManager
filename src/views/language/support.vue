<template>
  <div class="language-container">
    <div class="select-box">
      <add-btn @click.native="addTransfer">添加语言</add-btn>
    </div>
    <el-table :data="transferData" v-loading="tableLoading" class="data-list"
    border stripe>
      <el-table-column type="index"></el-table-column>
      <!-- 语言 -->
      <el-table-column label="语言" prop="name"></el-table-column>
      <!-- 区域 -->
      <el-table-column label="语言编码" prop="language_code"></el-table-column>
      <!-- 是否默认 -->
      ...
      <el-table-column label="操作" width="230">
        <template slot-scope="scope">
          ...
          <!-- 设为默认 -->
          <el-button v-if="scope.row.is_default === 0" class="btn-deep-purple" @click="setDefault(scope.row.id)">设为默认</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>

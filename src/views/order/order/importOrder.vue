<template>
  <div>
    <div style='color: #ffa943;margin-bottom: 16px'>{{ $t('批量入库需要填写入库的重量和尺寸，代表包裹已经收件并称重') }}</div>
      ...
      <el-form label-position="top">
        ...
        <el-form-item>
          ...
          >
            ...
            <!-- 商品名称 -->
            <el-table-column :label="$t('商品名称')" prop="package_name"> </el-table-column>
            <el-table-column :label="$t('快递单号')" prop="express_num"></el-table-column>
            <el-table-column
              :label="$t('重量') + `${localization.weight_unit || ''}`"
              prop="package_weight"
            ></el-table-column>
            <el-table-column
            ...
            <el-table-column :label="$t('操作')" width="260">
              <template slot-scope="scope">
                ...
                <el-button
                  class="btn-green"
                  @click="detailed(scope.row.details, scope.row.user_id)"
                >{{ $t('详细') }}</el-button
                >
                <!-- 删除 -->
                <el-button class="btn-light-red" @click="deleteTrack(scope.$index, logisticsList)">{{
                    $t('删除')
                  }}</el-button>
              </template>
            </el-table-column>

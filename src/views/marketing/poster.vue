<template>
  <div class="poster-container">
    ...
    <div class="poster-left">
      ...
      <div
        class="left-top"
        v-if="this.backgroundList.display_user_avatar === 1"
        style="position: relative"
      >
        <img src="../../assets/user.png" />
      </div>
      <h3 v-if="this.backgroundList.display_user_name === 1" style="position: relative">
        {{ $t('张三') }}
      </h3>
      <p v-if="this.backgroundList.display_share_info === 1" style="position: relative">
        {{ backgroundList.share_info }}
      </p>
      <div class="left-bottom">
        <img
          src="../../assets/code.png"
          :style="{
            width: backgroundList.avatar_size + 'px',
            height: backgroundList.avatar_size + 'px'
          }"
        />
      </div>
    </div>
    <div class="poster-right">
      <h4>{{ $t('设置面板') }}</h4>
      <el-form :model="backgroundList" ref="ruleForm" class="demo-ruleForm" label-width="140px">
        <!-- 开启用户头像 -->
        ...
        <el-form-item :label="$t('背景图像')">
          ...
          <span class="suggest-btn">{{
            $t('背景图像上传最多不超过五张，建议尺寸750px*1334px')
          }}</span>
        </el-form-item>
        <!-- 分享链接自定义图片 -->
